...
class Mysql
{
	...
	function Mysql($dbuser = '', $dbpassword = '', $dbname = '', $dbhost = 'localhost', $pconnect = false, $printerror = true)
	{
		...
		$this->conn = $pconnect ? @mysqli_pconnect ( $dbhost, $dbuser, $dbpassword ) : @mysqli_connect ( $dbhost, $dbuser, $dbpassword, true );

		if(! $this->conn)
			$this->error ( 'Connect database failed! The dbuser, dbpassoword or dbhost not correct.' );

		@mysqli_query ( "SET NAMES '" . $this->dbcharset . "'", $this->conn ); // 使用UTF8存取数据库, mysql 4.1以上支持
		@mysqli_query ( "SET sql_mode=''", $this->conn ); // 设置sql_model
	...
	function exe($query)
	{
		...
		$this->query_id = @mysqli_query ( $query, $this->conn );
		if(! $this->query_id){
	...
	{
		return @mysqli_fetch_array ( $queryId, MYSQL_ASSOC ); // 返回二维数组
	}
