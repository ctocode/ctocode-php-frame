...
require ('mp3.php');
$AE = new AudioExif ();
$file = 'BritneySpears-AutumnGoodbye.mp3';
print_r ( $AE->GetInfo ( $file ) );

$thisdir = dir ( "BritneySpears-AutumnGoodbye.mp3" );
	...
	function set_info($file, $pa)
	{
		...
		while(($tmp = $this->_get_head_frame ()) && ($head_num > 0)){
			...
			if($tmp['id'] == '3326b2758e66cf11a6d900aa0062ce6c'){ // Standard Info
				...
				// left length
				if($tmp['len'] > 0)
					$st_body2 .= $this->_read_head_buf ( $tmp['len'] );
				...
				// left buf?
				if($tmp['len'] > 0)
					$et_body .= $this->_read_head_buf ( $tmp['len'] );
				...
				if($tmp['len'] > 24)
					$head_body .= $this->_read_head_buf ( $tmp['len'] - 24 );
			}
		}
...
class _Mp3Exif extends _AudioExif
{
	...
	function _get_v1_info()
	{
		...
		if($this->head1['Reserved'] == 0)
			$ret['Track'] = $this->head1['Track'];
		else
			$ret['Description'] .= chr ( $ret['Reserved'] ) . chr ( $ret['Track'] );
		
		// Genre_idx
		$g = $this->head1['Genre'];
		if(! isset ( $this->genres[$g] ))
			$ret['Genre'] = 'Unknown';
		else
			$ret['Genre'] = $this->genres[$g];
	...
	function _get_v2_info()
	{
		...
		while(true){
			...
			if($k = $items[$tmp['fid']]){ // If first char is "\0", just skip
				if(substr ( $tmp['dat'], 0, 1 ) == "\0")
					$tmp['dat'] = substr ( $tmp['dat'], 1 );
				$ret[$k] = $tmp['dat'];
			}
		}
	...
	function _set_v1_info($pa)
	{
		...
		if(isset ( $pa['Track'] ))
			$tmp['Track'] = intval ( $pa['Track'] );
		
		// pack the data
		$buf = pack ( 'a3a30a30a30a4a28CCC', $tmp['id'], $tmp['Title'], $tmp['Artist'], $tmp['AlbumTitle'], $tmp['Year'], $tmp['Description'], 0, $tmp['Track'], $tmp['Genre'] );
	...
	function _set_v2_info($pa)
	{
		...
		$head_buf = pack ( 'a3CCCCCCC', $tmp['id'], $tmp['ver'], $tmp['rev'], $tmp['flag'], $size[1], $size[2], $size[3], $size[4] );
		$head_buf .= $head_body;
		$this->_file_save ( $head_buf, $old_len, $new_len );
	}
}
